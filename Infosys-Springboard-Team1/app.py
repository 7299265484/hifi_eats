...
app.config['MAIL_USERNAME'] = 'gokul040106@gmail.com'  # Replace with your email
app.config['MAIL_PASSWORD'] = 'gfzf hzvk ppvb ifcx'  # Replace with your email password
app.config['MAIL_DEFAULT_SENDER'] = 'gokul040106@gmail.com'  # Replace with your email
...
def signin():
    if request.method == 'POST':
        email = request.form.get('phone-email')  # Changed to match your form field
        ...
        if user and verify_password(password, user['password_hash']):
            ...
            cursor.execute('SELECT role_name FROM roles WHERE role_id = ?', (user['role_id'],))
...
def assign_role():
    ...
    if request.method == 'POST':
        ...
        cursor.execute('UPDATE users SET role_id = ? WHERE email = ?', (role_id, email))
        conn.commit()
        conn.close()
